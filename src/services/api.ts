...
const API_BASE_URL =
  ...
  "https://ec2-13-61-104-235.eu-north-1.compute.amazonaws.com:5001/api";
...
export const apiConfirmSignupCode = async (email: string, token: string) => {
  const response = await apiClient.post("/v1/Auth/confirm-signup-code", {
    email,
    token,
  });
  return handleResponse<{ message: string }>(response);
...
export const apiUploadServiceImage = async (
  serviceId: number,
  imageFile: File
) => {
  try {
...
export const apiUpdateService = async (
  id: string | number,
  serviceData: ServiceUpdateDto
) => {
  const response = await apiClient.put(`/v1/Service/${id}`, serviceData);
  return handleResponse<{ message: string }>(response);
};

export const apiPatchService = async (
  id: string | number,
  partialData: ServicePatchDto
) => {
...
export const apiDeleteServiceImage = async (
  serviceId: number,
  imageId: number
) => {
  try {
    const response = await apiClient.delete(
      `/v1/Service/${serviceId}/images/${imageId}`
    );
    return handleResponse<{ message: string }>(response);
  } catch (error) {
...
export const apiGetSellerBookings = async () => {
  try {
    const response = await apiClient.get(
      "/v1/Booking/seller?includeDetails=true"
    );
          ...
          if (!booking.buyerUsername && booking.userId) {
            try {
              ...
              const userResponse = await apiClient.get(
                `/v1/User/${booking.userId}`
              );
              const userData = handleResponse<any>(userResponse);
              booking.buyerUsername =
                userData.username || `User ${booking.userId}`;
            } catch (userErr) {
              console.error(
                `Error fetching user details for booking ${booking.id}:`,
                userErr
              );
...
export const apiUpdateBookingDate = async (
  id: number | string,
  newDate: string
) => {
  try {
...
export const apiGetReviewByBookingId = async (bookingId: number | string) => {
  try {
    ...
    const matchingReview = userReviews.find(
      (review) => review.bookingId === Number(bookingId)
    );

    console.log(
      `Checking for review for booking ${bookingId}:`,
      matchingReview || "No review found"
    );
...
export const apiGetServiceReviews = async (serviceId: number) => {
  try {
    ...
    console.log(
      `Fetched ${reviews.length} reviews for service ${serviceId}:`,
      reviews
    );
    return reviews;
  } catch (error) {
...
export const apiReactivateAccountByToken = async (
  email: string,
  token: string
) => {
  try {
    console.log(
      "Attempting to reactivate account with email:",
      email,
      "and token:",
      token
    );
