...
const ReviewList: React.FC<ReviewListProps> = ({
  ...
  const eligibleBookings = userBookings.filter(
    (booking) =>
      Number(booking.serviceId) === Number(serviceId) &&
      (booking.status === "Completed" || booking.status === "Paid")
  );

  // Check if user has already reviewed this service
  const hasUserReviewed = reviews.some(
    (review) => review.userId === (user?.id ? Number(user.id) : null)
  );
  ...
  const isSellerOwnService =
    isSeller && user?.id && serviceDetails?.sellerId === Number(user.id);
          ...
          {!editingReview &&
            ...
            eligibleBookings.length === 0 && (
              <div className="mb-6 p-4 bg-yellow-50 rounded-md text-yellow-700">
                You need to book and complete this service before you can leave
                a review.
              </div>
            )}
